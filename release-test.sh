...
if [ "$GITHUB_ACTIONS" = "true" ]; then
    ...
    repo=https://github.com/common-workflow-language/cwlprov-py.git
    HEAD=$(git rev-parse HEAD)
fi
pipver=23.1  # minimum required version of pip for Python 3.12
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null && pwd )"
run_tests() {
for RO in ${DIR}/test/* ; do
  python -m cwlprov.tool -d ${RO} validate && \
  python -m cwlprov.tool -d ${RO} info && \
  python -m cwlprov.tool -d ${RO} who && \
  python -m cwlprov.tool -d ${RO} prov && \
  python -m cwlprov.tool -d ${RO} inputs && \
  python -m cwlprov.tool -d ${RO} outputs && \
  python -m cwlprov.tool -d ${RO} runs && \
  python -m cwlprov.tool -d ${RO} runtimes
done
}
